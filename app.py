...
class Inspect:
    ...
    def __inspect():
        ...
        stats = list(map(str.split, Inspect.__exe("docker stats --no-stream | awk 'NR-1 > 0 { print $3, $4 }'").splitlines()))
        ...
        for gpu, proc in zip(map(int, devices[::2]), devices[1::2]):
            yield (gpu, [proc, *next(c for c, ps in zip(containers, processes) if proc in ps), *stats[gpu], *utils[gpu]])
